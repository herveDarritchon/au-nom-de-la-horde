...
description: >
  Génère un plan technique versionnable à partir d'une issue GitLab ou d'un
  cadrage, puis l'écrit sous `documentation/plan/<business-domain-path>/`.
argument-hint: "[issue-url|issue-number|cadrage]"
model: sonnet
effort: medium
compatibility: "claude-code;opencode"
metadata:
  project: tor2e
  scope: planification technique à partir d'une issue ou d'un cadrage
disable-model-invocation: true
disallowed-tools: Agent
---

# Plan depuis une issue gitlab

Utilise ce skill quand l'utilisateur demande de créer un plan d'implémentation à partir d'une issue gitlab, d'une URL
d'issue, d'un numéro d'issue, ou d'un cadrage.

> Langue : produis le plan en français sauf demande explicite contraire.

## 1. Mission

Produire un plan d'implémentation court, actionnable, puis l'écrire directement comme fichier Markdown dans une
arborescence métier sous `documentation/plan/`.
...
3. Ne jamais modifier les issues gitlab.
4. Ne jamais lancer de tests.
5. Ne jamais créer de branche, commit ni PR.
6. Ne pas élargir le périmètre de l'issue.
7. Lire `documentation/plan/` pour respecter le format, identifier les chemins métier existants et éviter les
   collisions.
...
14. Si l'issue ou le cadrage est insuffisant pour produire un plan fiable, arrêter et retourner les ambiguïtés au lieu
    d'inventer.
...
Si l'entrée est une URL gitlab mais que les outils web/bash sont indisponibles, utilise uniquement le texte visible dans
l'entrée. Si le contenu de l'issue n'est pas accessible et que le titre ne suffit pas, bloque avec une question courte.
